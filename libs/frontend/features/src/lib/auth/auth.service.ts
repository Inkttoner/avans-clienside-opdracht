...
export class AuthService {
  ...
  login(email: string, password: string): Observable<IUser| undefined> {
    ...
    return this.http
      ...
      map((response) => {
        ...
        if (user && user._id) { // Check if the response contains valid user data
  ...
  register(userData: IUserRegistration): Observable<IUserRegistration | undefined> {
    console.log(`register at ${environment.dataApiUrl}/users`);
    console.log(userData);
    return this.http
            .post<IUserRegistration>(`${environment.dataApiUrl}/user`, userData)           
      .pipe(
        map((userData) => {
  ...
  getUserFromLocalStorage(): Observable<IUser | undefined> {
    ...
    if (localUser) {
      return of(JSON.parse(localUser) as IUser);
    } else {
      return of(undefined);
    }
  }
